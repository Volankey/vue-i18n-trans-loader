...
const _processTranslate = async ({
  ...
  if (matches) {
    for (let match of matches) {
      ...
      for (let key of keys) {
        //拿到$t()中间的文字
        const willTranslateKey = key.substring(4, key.length - 2);
          ...
          for (let j = 0, len = langIndexs.length; j < len; j++) {
            ...
            const targetLang = options.langs[j].name;
                const ifPrevent = utils.ifPreventResetFn(options, willTranslateKey);
                  ...
                  const flag = utils.ifExistInResetFileFn(options, { targetLang, willTranslateKey });
            ...
            //如果是source的索引
            if (j === 0) {
              langIndex[willTranslateKey] = willTranslateVal || willTranslateKey;
            } else if (!hasTrans[options.langs[j].name][willTranslateKey] && !langIndex[
              willTranslateKey]) {
              try {
                langIndex[willTranslateKey] = await options.translate(sourceLang, targetLang,
              ...
              langIndex[willTranslateKey] = hasTrans[options.langs[j].name][willTranslateKey] ||
